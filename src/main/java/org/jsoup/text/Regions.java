 ...
 *  become invalid, and removing a region certainly does. (In general,
 *  a Point can become invalid when you operate on an earlier part of
 *  its TextNode.) To avoid unpleasantness, this class provides a
 *  safelyModifiable() Regions that merges partly overlapping regions
 *  and returns a result whose first Region is last in the document.
 *
 *  <p>Note that when you modify the document using a Region, other
 *  overlapping Regions may be invalidated.
 */
public class Regions extends ArrayList<Region> {
    ...
    /**
     *  Add the named class to each Element covering text in any of
     *  these Regions, after splitting by Element.
     *
     *  <p>This loops over the result of {@link #splitByElements()}
     *  and calls {@link Region#addClass(String)} on each.
     *
     *  @return The possibly split Regions from this, for chaining.
     */
    public Regions addClass(final String className) {
        Regions result = splitByElements();
        for(Region r : result)
    ...
    /**
     *  Remove the named class from each Element covering text in
     *  any of these Regions, after splitting by Element.
     *
     *  <p>This loops over the result of {@link #splitByElements()}
     *  and calls {@link Region#removeClass(String)} on each.
     *
     *  @return The possibly split Regions from this, for chaining.
     */
    public Regions removeClass(final String className) {
        Regions result = splitByElements();
        for(Region r : result)
