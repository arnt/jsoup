 ...
 *
 * <p>There is also a (slow) Comparator that compares Points by
 * position within the document, and there are a few utiliy functions.
 */
public class Point {
    private TextNode in;
    private int offset;
    ...
    public Point rightBound(final Node within) {
        ...
        if(offset < in.getWholeText().length())
            return this;
        TextNode candidate = in;
        Region.NeighbouringTextNode next =
            Region.NeighbouringTextNode.next(candidate, within);
        while(next.textNode != null &&
              next.textNode.getWholeText().isEmpty()) {
            candidate = next.textNode;
            next = Region.NeighbouringTextNode.next(candidate, within);
        }
    ...
    void splitAfterUpTo(final Node grandParent) {
        ...
        if(n.parent() != grandParent)
            throw new IllegalStateException(
                "splitAfterUpTo: grandParent is not an ancestor; " +
                "split walked past the document root.");
    }
    ...
    /**
     *  This comparator can be used to sort points by position; a
     *  point earlier in the document is smaller than a point later.
     *
     *  Note that using this comparator is likely to be very slow for
     *  large documents, since it often walks from the points involved
     *  up to the document root.
     */

    static public Comparator<Point> byPosition =
        new Comparator<Point>() {
            ...
            public int compare(final Point p1, final Point p2) {
                ...
                p2.revalidate();
                if(p1.in == p2.in) {
                    // Integer.compare is new in 1.7; this targets 1.5
                ...
                Node shared = p1.getSharedParent(p2);
                if(shared == null)
                    throw new IllegalStateException();
                Node c1 = p1.getChildWithin(shared);
                Node c2 = p2.getChildWithin(shared);
                if(c1.siblingIndex() > c2.siblingIndex())
                    return 1;
                return -1;
            }
        };
    ...
    /**
     *  Return an ArrayList of parents, where the point's immediate
     *  parent (a TextNode) is first and the root is last.
     */
    ...
    /**
     *  Return a Node of that contains both this point and the other,
     *  or null if there is no such Node (ie. the two are not in the
     *  same document).
     *
     *  This function can be surprisingly slow if the document is deep.
     */

    public Node getSharedParent(final Point other) {
        ArrayList<Node> parents = other.getParents();
        Node n = in;
        while(n != null) {
    ...
    /**
     *  Return a Node of that contains this point and is a child of
     *  the specified parent, o null if the point is not within the
     *  parent.
     *
     *  This function can be surprisingly slow if the document is deep.
     */

    public Node getChildWithin(final Node parent) {
        if(parent == in)
            return null;
        Node n = in;
