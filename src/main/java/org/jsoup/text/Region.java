 ...
 *  can avoid such deep changes using splitByBlockElements() and
 *  splitByElements(). The addClass() and removeClass() modifiers take
 *  care to only add spanss, not split block elements.
 *
 *  <p>Region provides two Comparators, byStart and byEnd, that compare
 *  by the position of the start and end points.
 *
 *  <p>Note that when you modify the document using a Region, other
 *  overlapping Regions may be invalidated. (In general, a Point can
 *  become invalid when you operate on an earlier part of its
 *  TextNode.)
 */
public class Region {
    private Point start;
    ...
    /**
     *  Return the Elements covering all of this Region, and
     *  covering no other text.
     *
     *  <p>This modifies the document by splitting text nodes and/or
     *  inserting span elements until the result is well defined.
     */
    ...
    /**
     *  Add the named class to each Element returned by {@link
     *  #parentElements()}. Since {@link #parentElements()} may split
     *  the parent Element so that the affected Elements cover only
     *  this Region's text, this may change the document structure.
     *
     *  @return this Region, for chaining.
     */
    public Region addClass(final String className) {
    ...
    /**
     *  Remove the named class from each Element returned by {@link
     *  #parentElements()}. Since {@link #parentElements()} may split
     *  the parent Element so that the affected Elements cover only
     *  this Region's text, this may change the document structure.
     *
     *  @return this Region, for chaining.
     */
    public Region removeClass(final String className) {
        for(Element e : parentElements())
    ...
    public void ensureRemovability() {
        ...
        if(before != start && before.getTextNode() != null
           && hasAncestor(before.getTextNode(), common))
            before.splitAfterUpTo(common);
    }

    private static boolean hasAncestor(final Node node, final Node ancestor) {
        Node n = node;
    ...
    static public Comparator<Region> byStart =
        new Comparator<Region>() {
            @Override
            public int compare(final Region r1, final Region r2) {
                return Point.byPosition.compare(r1.getStart(), r2.getStart());
            }
        };


    static public Comparator<Region> byEnd =
        new Comparator<Region>() {
            @Override
            public int compare(final Region r1, final Region r2) {
                return Point.byPosition.compare(r1.getEnd(), r2.getEnd());
            }
        };
