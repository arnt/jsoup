...
public class PointTest {
    ...
    public void findsParagraphLimits() {
        ...
        Point atStart = Point.atStartOfElement(empty);
        assertEquals(0, atStart.getOffset());
        assertEquals(empty, atStart.getTextNode().parent());
        assertEquals(1, empty.childNodeSize());
    }

    @Test
    public void canOrderByPosition() {
        Document html = Parser.parseBodyFragment(LONG_DOCUMENT, "");
        Point across1 = html.find("Across").first().getStart();
        Point across2 = html.find("Across").first().getEnd();
        assertEquals(0, Point.byPosition.compare(across1, across1));
        assertEquals(-1, Point.byPosition.compare(across1, across2));
        assertEquals(1, Point.byPosition.compare(across2, across1));
        Point before = html.find("Before").first().getEnd();
        assertEquals(1, Point.byPosition.compare(before, across1));
        assertEquals(1, Point.byPosition.compare(before, across2));
        Point butThat = html.find("But that").first().getEnd();
        assertEquals(-1, Point.byPosition.compare(before, butThat));
        Point yourBeach1 = html.find("your beach").first().getStart();
        Point yourBeach2 = html.find("your beach").first().getEnd();
        Point yourBeach3 = yourBeach2.rightBound();
        assertEquals(-1, Point.byPosition.compare(butThat, yourBeach1));
        assertEquals(-1, Point.byPosition.compare(butThat, yourBeach2));
        assertEquals(-1, Point.byPosition.compare(butThat, yourBeach3));
        assertEquals(-1, Point.byPosition.compare(yourBeach1, yourBeach2));
        assertEquals(-1, Point.byPosition.compare(yourBeach2, yourBeach3));
        assertEquals(-1, Point.byPosition.compare(before, butThat));
        Point yourHappy = html.find("your happy").first().getStart();
        assertEquals(-1, Point.byPosition.compare(yourBeach2, yourHappy));
        assertEquals(-1, Point.byPosition.compare(yourBeach3, yourHappy));
        assertEquals(-1, Point.byPosition.compare(butThat, yourHappy));
    }


    @Test
    public void wrapWhenStartAnchorIsEmptyTextNode() {
        Document html = new Document("");
        Element body = html.appendElement("html").appendElement("body");
        Element bp = body.appendElement("div").addClass("bodypart");

        bp.appendElement("p").text("Hei");
        bp.appendElement("div").appendChild(new TextNode(""));
        bp.appendElement("p").text("paragraph two");
        bp.appendElement("div").appendChild(new TextNode(""));
        bp.appendElement("p").text("paragraph three");
        bp.appendElement("div").appendChild(new TextNode(""));
        bp.appendElement("p").text("Helsing Ivar");

        Element ltr = bp.appendElement("div").attr("dir", "ltr");
        Element header = ltr.appendElement("span");
        header.appendChild(new TextNode("From: Arnt <arnt@gulbrandsen.priv.no>"));
        header.appendElement("br");
        header.appendChild(new TextNode("Sent: Monday"));
        header.appendElement("br");
        header.appendChild(new TextNode("Subject: Re: runes"));
        ltr.appendChild(new TextNode(""));
        ltr.appendElement("div").appendChild(new TextNode(""));

        Element quote = bp.appendElement("div")
                          .attr("style", "font-size:0.917rem;");
        quote.appendElement("p").text("Flott! Takk! Quoted reply text.");
        quote.appendElement("p").text("Continuation paragraph.");
        quote.appendElement("p").text("Arnt");

        Region r = bp.find("Flott! Takk! Quoted reply text.").first();
        assertNotNull(r, "test setup: region should be findable");

        Element wrapper = r.wrap("<blockquote>");
        assertNotNull(wrapper,
                      "wrap returned null because Region.find anchored " +
                      "the start Point on an empty TextNode and " +
                      "rightBound() didn't skip past it.");
    }
}
