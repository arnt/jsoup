...
public class RegionsTest {
    ...
    @Test
    public void addsClassPerElement() {
        Document html = Jsoup.parse("<p>Partly em<b>bold</b>ened words.");
        Element p = html.select("p").first();

        // a single region spanning multiple Elements is first split
        // by Element, and each part's parent Element receives the
        // class -- inserting a span around any text nodes that need
        // their own Element wrapper.
        p.find("emboldened").addClass("hl");
        assertEquals("<p>Partly <span class=\"hl\">em</span><b class=\"hl\">bold</b><span class=\"hl\">ened</span> words.</p>",
                     p.outerHtml());
    }

    @Test
    public void addsClassToMultipleRegions() {
        Document html = Jsoup.parse("<p>word blah <b>word</b> blah word");
        Element p = html.select("p").first();
        p.find("word").addClass("hl");
        assertEquals("<p><span class=\"hl\">word</span> blah <b class=\"hl\">word</b> blah <span class=\"hl\">word</span></p>",
                     p.outerHtml());
    }
